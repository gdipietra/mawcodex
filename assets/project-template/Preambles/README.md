...
The `scripts/check-palette-sync.py` script greps both files and reports any divergence:

```bash
python scripts/check-palette-sync.py
```

It's also invoked (non-blocking) from `python scripts/validate-project.py`.
...
3. Run `python scripts/check-palette-sync.py` — it should report "in sync".
