<!-- Adapted from templates/tikz-snippets/README.md at be53c12f235996dff41fb7f21580506fd2dd8d50; source SHA-256 2e858f3f15de274cd31543eb7aa4903193c2001869f2b2181938e9fe023c4834. -->
...
- Satisfies [`tikz-prevention.md`](../../../references/rules/tikz-prevention.md): explicit node dimensions (P1), coordinate map comment (P2), no `scale=` (P3), directional keyword on every edge label (P4).
- Uses color names from [`Preambles/header.tex`](../../project-template/Preambles/header.tex) once TX2 ships. Until then, snippets embed their palette inline.
...
4. **When in doubt, run the six-pass check.** See [`tikz-measurement.md`](../../../references/rules/tikz-measurement.md) for formulas. The `$extract-tikz` and `$new-diagram` skills run the checks for you.
...
- Passes the P3 and P4 grep pre-checks in [`tikz-prevention.md`](../../../references/rules/tikz-prevention.md) by construction.
...
When contributing a general-purpose snippet, use the repository's documented
