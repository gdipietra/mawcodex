<!-- Adapted from .claude/rules/orchestrator-protocol.md at be53c12f235996dff41fb7f21580506fd2dd8d50; source SHA-256 dd71f057297541a8685a7c873043d7b74c6cf3bbce994f5adecdbfeccd11b287. -->
...
**The review-fix loop is a real runtime contract, expressed with the primitive every Codex session has: a bounded subagent.** Skills fan out to forked reviewers, reduce their *structured* findings ([`orchestration-schemas.md`](../orchestration-schemas.md)) through a deterministic gate, judge with a hallucination guard, and loop until dry. What is *not* automatic is the **trigger**: nothing launches this loop on its own — the user (or a skill invocation) starts it. That boundary is deliberate (see "What is NOT automatic").
...
These four primitives are the runtime. Every fan-out skill is a composition of them; none should re-describe them in prose — they reference this section and [`orchestration-schemas.md`](../orchestration-schemas.md).

### 1. Fan-out

Spawn the reviewers **in parallel in a single message** — N a bounded subagent calls, each `isolated context` so the main thread stays clean and each reviewer gets full budget for its lens. bounded subagents are the **portable primitive**: they exist in every Codex install, so the template depends on them, not on the session-gated Workflow tool. *(Where the Workflow tool is available — e.g. an `ultracode`/dynamic-workflow session — a skill may use it for the same fan-out→reduce→judge shape; treat that as an optional accelerator, never a requirement.)*

Which agent fills which lens, at which model tier, is in [`agent-fleet.md`](../agent-fleet.md).
...
A synthesizer/editor may freely *downgrade* or *de-duplicate* lens findings, but any **CRITICAL it introduces that no lens raised** must survive the post-judge hallucination gate ([`orchestration-schemas.md` §4](../orchestration-schemas.md)): re-verify it in a fresh `claim-verifier` fork; if it can't be grounded, drop it to `[JUDGE-HALLUCINATED]` and recompute. This is what makes an autonomous review trustworthy next to a credibility-sensitive artifact.
...
A isolated subagent cannot stop to ask the user a question. So every interactive choice a fan-out needs — target journal, sampled dispositions, peeve budget, N referees, fresh-context flag, cross-artifact/novelty toggles — is gathered **before** the fleet spawns, echoed back as the **Pre-Flight Report**, and only then launched. Schema: [`orchestration-schemas.md` §5](../orchestration-schemas.md). An unresolved required field (e.g. an unknown journal) halts *before* launch, never mid-run. This is what lets `--peer`, `--variance`, and `editor` disambiguation keep their interactivity inside a no-mid-run-input runtime.
...
- **Do NOT treat this phrase as commit authorization.** Commits require an explicit `$commit` or unambiguous request — see [`skills/commit/SKILL.md`](../../skills/commit/SKILL.md).

## Cross-references

- [`references/orchestration-schemas.md`](../orchestration-schemas.md) — FINDING / SCORECARD / RUN_CONFIG / hallucination-gate contracts.
- [`references/agent-fleet.md`](../agent-fleet.md) — the reviewer fleet + model tiers.
- [`references/rules/plan-first-workflow.md`](plan-first-workflow.md) — when to enter plan mode before invoking a skill.
- [`references/rules/quality-gates.md`](quality-gates.md) — threshold definitions + the pre-commit hook.
- [`references/rules/post-flight-verification.md`](post-flight-verification.md) — the forked-verifier mechanism the hallucination gate reuses.
- [`references/rules/cross-artifact-review.md`](cross-artifact-review.md) — paper ↔ code dependency-graph pattern.
