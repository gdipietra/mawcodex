<!-- Adapted from .claude/rules/confidential-data.md at be53c12f235996dff41fb7f21580506fd2dd8d50; source SHA-256 b3ec4f5bd127afd7a91cea477d335e8b77867eb31049093ad1d5629d534e466f. -->

## Applicability

Load this rule for: `data/**`, `**/*.dta`, `**/*.sav`, `**/raw/**`, `**/restricted/**`, `**/confidential/**`.
...
- **Feature branches per author**, merged via PR — the same flow the rest of
  the template uses; no author force-pushes shared history (the
  [MAW Codex guardrail hook](../../hooks/scripts/maw_hook.py) blocks
  `--force` and blanket staging, two common ways sensitive files enter
  history).
- **`MEMORY.md` syncs via git; `.codex/state/personal-memory.md` stays local** (`meta-governance.md`). Machine-specific restricted-data *paths* live in personal-memory, never in committed files.
...
- Enable strict path-leak protection while in restricted dirs: `MAWCODEX_STRICT_PATHS=1` makes `git-guardrails` *deny* (not just warn) hardcoded machine paths in code.
...
- [`references/rules/replication-protocol.md`](replication-protocol.md) — passport entries note disclosure status.
- [`references/rules/meta-governance.md`](meta-governance.md) — the committed-vs-local two-tier model this builds on.
