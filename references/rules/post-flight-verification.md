<!-- Adapted from .claude/rules/post-flight-verification.md at be53c12f235996dff41fb7f21580506fd2dd8d50; source SHA-256 bf76978f5bcde43bb6b51ce0a1897511c034b425e86568924acb2791308f18cb. -->

## Applicability

Load this rule for: `skills/lit-review/SKILL.md`, `skills/research-ideation/SKILL.md`, `skills/respond-to-referees/SKILL.md`, `skills/review-paper/SKILL.md`, `skills/interview-me/SKILL.md`.
...
Symmetric partner to **Pre-Flight Reports** (`references/rules/content-invariants.md` + skill-level `## Phase 0`). Where Pre-Flight proves inputs were read *before* work, Post-Flight proves the output's factual claims hold *after* drafting — before the skill returns to the user.

**Adapted from:** Dhuliawala et al. 2023, "Chain-of-Verification Reduces Hallucination in Large Language Models" ([arXiv:2309.11495](https://arxiv.org/abs/2309.11495)). The **independence trick** — answer verification questions in a context that does not contain the original draft — is architecturally enforced here by running `claim-verifier` via a bounded subagent with `isolated context`. The forked agent literally cannot self-confirm; it has never seen the draft.
...
Spawn a bounded subagent assigned the `claim_verifier` custom-agent role, or
give a fresh subagent the portable
[`claim-verifier`](../agent-roles/claim-verifier.md) instructions. Start it
without prior conversation turns when the active Codex surface supports that
isolation. Hand it only the claims, verification questions, and source
material pointers. **Do not include the draft.** If fresh-context isolation
cannot be established, disclose that limitation and treat the independence
check as UNVERIFIED.
...
- `references/agent-roles/claim-verifier.md` — the forked verifier.
- `skills/verify-claims/SKILL.md` — user-facing wrapper for ad-hoc verification of any text.
- `references/rules/content-invariants.md` — Pre-Flight (input side).
- `references/rules/cross-artifact-review.md` — pattern-based; Post-Flight is draft-based.
- `references/rules/summary-parity.md` — rule against enumerative summaries drifting from their bodies; Post-Flight is the factual equivalent for draft content.
