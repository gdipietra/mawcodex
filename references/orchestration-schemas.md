<!-- Adapted from .claude/references/orchestration-schemas.md at be53c12f235996dff41fb7f21580506fd2dd8d50; source SHA-256 3e240a00fc1b15a54f0ccbfbebbd1bbbd1f539d394899a12bcfcffa43e39d12b. -->
...
This is a **reference**, not a runtime: a Codex session has no JSON validator in the loop. The schemas are the *target shape* each reviewer subagent returns (as a fenced ```yaml block at the end of its report) and the synthesizer reads. See [`references/rules/orchestrator-protocol.md`](rules/orchestrator-protocol.md) for how the fan-out → reduce → judge → loop-until-dry runtime uses them.
...
3. Re-verify each candidate in a **fresh task context**—spawn a bounded
   `claim_verifier` subagent without prior conversation turns and give it only
   the claim plus the cited artifact location, per
   [`post-flight-verification.md`](rules/post-flight-verification.md).
...
run_config:
  ...
  novelty_check: true                     # editor web search probe (Post-Flight-verified)
...
- [`references/rules/orchestrator-protocol.md`](rules/orchestrator-protocol.md) — the runtime that consumes these schemas.
- [`references/agent-fleet.md`](agent-fleet.md) — which agent fills which lens, at which model tier.
- [`references/rules/post-flight-verification.md`](rules/post-flight-verification.md) — the forked-verifier mechanism the §4 gate reuses.
- [`references/rules/summary-parity.md`](rules/summary-parity.md) — the two-strikes rule the loop reuses for repeatedly-flagged findings.
